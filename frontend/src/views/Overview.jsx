...
export default function Overview({ onGoChat }) {
  ...
  // KPI tiles — live values with static fallbacks
  const KPI1 = [
    ...
    { cls: 'sb', label: 'Orders Today',            conf: '99%', val: String(d?.orders_today ?? 24),                    delta: `▲ ${d?.orders_dispatched ?? 18} dispatched · ${d?.orders_pending ?? 6} pending`, deltaClass: 'up', sub: `${d?.orders_dispatched ?? 18} dispatched · ${d?.orders_pending ?? 6} pending`, bar: 82, barColor: 'var(--b2)', q: 'How many orders did I receive today vs target? Which orders are pending?' },
  ...
  const targetData     = revChartData.map((v, i) => +(v * 1.08).toFixed(1));
  ...
  useEffect(() => {
    const d1 = createChart(revRef, {
      ...
      data: {
        ...
        datasets: [
          { data: revChartData, borderColor: '#0f766e', backgroundColor: gradientFill('#0f766e'), borderWidth: 2.5, tension: .4, pointRadius: 0, fill: true, label: 'Revenue' },
    ...
    return () => { d1(); d2(); d3(); };
  }, [d]); // re-render charts when live data arrives
    ...
    <div className="view">
      <div className="src-banner">
        ...
        <div className="src-text">
          <strong>StockSense AI reads directly from your Dealer Management System (DMS).</strong> All numbers below are pulled live from your existing inventory, billing, and order software — no manual entry required.
        </div>
      ...
      <div className="ai-banner">
        ...
        <div className="ai-b">
          <div className="ai-lbl">
            AI Daily Brief — {src === 'mysql' ? '● Live DB' : 'Demo data'} · Updated {new Date().toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit' })}
          </div>
      ...
      <div className="gl g75">
        <div className="card">
          <div className="ch"><div><div className="ctit">Revenue Trend — Last 12 Months</div><div className="csub">₹ Lakhs · From your billing system</div></div><DataSourceBadge source={src} /></div>
